  ...
  const [
    ...
    pool.query<PgPositionStatsRow>(`
      SELECT
        ...
        COUNT(*) FILTER (WHERE status = 'CLOSED')::text AS closed_positions,
        COUNT(*) FILTER (WHERE status = 'CLOSED' AND COALESCE(pnl_usd, 0) > 0)::text AS winners,
        COUNT(*) FILTER (WHERE status = 'CLOSED' AND COALESCE(pnl_usd, 0) < 0)::text AS losers,
        COALESCE(SUM(pnl_usd) FILTER (WHERE status = 'CLOSED'), 0)::text AS total_pnl_usd,
        COALESCE(AVG(pnl_usd) FILTER (WHERE status = 'CLOSED'), 0)::text AS avg_pnl_usd
      FROM live_positions
    `),
  ]);
