...
function isStalePositionStatus(value: string | null | undefined): boolean {
  const status = normalizedStatus(value);
  return status.startsWith("STALE_") || status.includes("NOT_ON_EXCHANGE") || status.includes("DUPLICATE_LOCAL");
}

function isActivePositionStatus(value: string | null | undefined): boolean {
  return normalizedStatus(value) === "OPEN";
}

function isStaleOrderStatus(value: string | null | undefined): boolean {
  const status = normalizedStatus(value);
  return (
    status.startsWith("STALE_") ||
    status.includes("UNCONFIRMED_EXPIRED") ||
    status.includes("NOT_ON_EXCHANGE") ||
...
function isExpiredUnconfirmedOrder(order: Order): boolean {
  const status = normalizedStatus(order.status);
  return status === "LIVE_UNCONFIRMED" && !isWithinHours(order.created_at, 10 / 60);
}

function statusClass(value: string | null | undefined): string {
  if (isStalePositionStatus(value) || isStaleOrderStatus(value)) return styles.staleText;
  const status = normalizedStatus(value);
  if (status === "CLOSED" || status.includes("VERIFIED")) return styles.goodText;
  if (status.includes("UNCONFIRMED")) return styles.warnText;
  return "";
}
...
export default function StrikebotDashboard({ token }: { token: string }) {
  ...
  const visibleOrders = useMemo(() => {
    return (data?.recentOrders ?? []).filter((order) => {
      ...
      if (isStaleOrderStatus(status)) return false;
  ...
  const hiddenStalePositions = useMemo(() => {
    return visiblePositions.filter((position) => isStalePositionStatus(position.status));
  }, [visiblePositions]);
  ...
  const stats = useMemo(() => {
    ...
    const allClosedPositions = activeOrClosedPositions.filter((position) => normalizedStatus(position.status) === "CLOSED");
        ...
        <article className={`${styles.panel} ${styles.rulesPanel}`}>
          ...
          <div className={styles.rulesCompact}>
            ...
            <div>
              ...
              <strong className={burstSummary.active ? styles.warnText : undefined}>
                ...
                <tbody>
                  {visibleOrders24h.length === 0 ? (
                    ...
                    visibleOrders24h.slice(0, 20).map((order) => (
                      <tr key={order.id}>
                        <td>{order.id}</td>
                        <td className={statusClass(order.status)}>{order.status}</td>
